...
| customer.pix_key               |  Yes (only for Withdrawal pix) | string                      | User's Pix Key that will receive the payment. Required only for Orders of type Pix Withdrawal.    
| customer.pix_key_type               |  Yes (only for Withdrawal pix) | string                      |User's Pix key type. Required only for Orders of type Pix Withdrawal. Accepted values:<br> "cpf"= User's document<br> "phone"= User's phone number. Registered in the PIX system. It may be different from the user's registration phone number<br>"email"= User's email. Registered in the PIX system. It may be different from the user's registration email<br>"random_key"= User's random key                                                                             |
